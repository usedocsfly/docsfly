...
export async function createApp(projectDirectory?: string, options: CreateAppOptions = { template: 'default' }) {
  ...
  function detectPackageManager(): string {
    ...
    throw new Error('Could not detect package manager. Please run with npm, yarn, pnpm, or bun.')
  }
  ...
  console.log(chalk.cyan(`Creating a new Docsfly app in ${chalk.green(projectPath)}`))
  console.log()
  ...
  if (!options.skipInstall) {
    ...
    try {
      execSync(`${packageManager} install`, { cwd: projectPath, stdio: 'pipe' })
